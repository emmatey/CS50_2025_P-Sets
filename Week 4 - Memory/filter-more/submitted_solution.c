...
RGBTRIPLE sum_valid_neighbors(int i, int j, int height, int width, RGBTRIPLE image[height][width])
{
    float divisor = 0;
    float blu = 0;
    float grn = 0;
    ...
    for (int row = -1; row < 2; row++)
    {
        ...
        for (int row_idx = -1; row_idx < 2; row_idx++)
        {
            if ((row + i) >= 0 && (row + i) < height && (row_idx + j) >= 0 && (row_idx + j) < width)
            {
                blu += image[row + i][row_idx + j].rgbtBlue;
                grn += image[row + i][row_idx + j].rgbtGreen;
                red += image[row + i][row_idx + j].rgbtRed;
                divisor++;
            }
        }
    }
...
RGBTRIPLE do_sobel(int i, int j, int height, int width, RGBTRIPLE image[height][width])
{
    float gxBlu = 0;
    float gxGrn = 0;
    float gxRed = 0;
